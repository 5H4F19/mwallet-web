...
import { EVM_FAMILY, getApiNode, loadWalletModule } from "../constants.js";
...
class WalletManager {
  ...
  tokensByPlatform(platform) {
    return [...this.#wallets.values()].filter((wallet) => {
      return (
        wallet.crypto.platform === platform && wallet.crypto.type === "token"
      );
    });
  }
...
export default class Account extends EventEmitter {
  ...
  get siteUrl() {
    return this.isOnion
      ? import.meta.env.VITE_SITE_URL_TOR
      ...
      if (user.email) {
        ...
        const hash = hex.encode(
          hmac(
            sha256,
            "Coin Wallet",
            hex.encode(this.#clientStorage.getDetailsKey())
          )
        );
  ...
  async init() {
    ...
    const cryptos = this.#details.getCryptos().map((local) => {
      const remote =
        this.#cryptoDB.get(local._id) ||
        (local.type === "token"
          ? this.#cryptoDB.getTokenByAddress(local.platform, local.address)
          : undefined);
    ...
    cryptos.forEach((crypto) => {
      if (crypto.type === "token" && crypto.supported === true) {
        const platform = cryptos.find(
          (item) => item.type === "coin" && item.platform === crypto.platform
        );
  ...
  async #openWallet(crypto, walletStorage, settings = undefined) {
    ...
    if (this.#clientStorage.hasPublicKey(crypto.platform)) {
      await wallet.open(
        this.#clientStorage.getPublicKey(crypto.platform, this.#deviceSeed)
      );
    } else {
  ...
  #getWalletOptions(crypto, platform, storage, settings) {
    ...
    if (crypto._id === "monero@monero") {
      options.wasm = new URL(
        "@coinspace/monero-core-js/build/MoneroCoreJS.wasm",
        import.meta.url
      ).href;
    }
    return options;
  ...
  async create(walletSeed, pin) {
    ...
    const walletId = hex.encode(
      await ed25519.getPublicKeyAsync(walletSeed, false)
    );
    ...
    const walletStorages = await WalletStorage.initMany(
      this,
      this.#details.getSupportedCryptos()
    );
    const wallets = await Promise.all(
      this.#details.getSupportedCryptos().map(async (crypto) => {
        const wallet = await this.#createWallet(
          crypto,
          walletSeed,
          walletStorages[crypto._id]
        );
        // save public key only for coins
        if (crypto.type === "coin") {
          this.#clientStorage.setPublicKey(
            crypto.platform,
            wallet.getPublicKey(),
            deviceSeed
          );
  ...
  async addWallet(crypto, walletSeed) {
    ...
    if (this.#clientStorage.hasPublicKey(crypto.platform)) {
      ...
      if (crypto.type === "token") {
        const platform = this.#cryptoDB.platform(crypto.platform);
        const walletStorages = await WalletStorage.initMany(this, [
          crypto,
          platform,
        ]);
        const wallet = await this.#createWallet(
        ...
        this.#wallets.set(
          await this.#openWallet(crypto, walletStorages[crypto._id])
        );
  ...
  #migrateV5Details() {
    ...
    if (cryptoSettings) {
      const platformSettings = Object.keys(cryptoSettings).reduce(
        (result, id) => {
    ...
    if (tokens) {
      ...
      const cryptos = [
        ...
        ...tokens.filter((crypto) => {
          return (
            crypto?._id?.includes("@") &&
            !defaultCryptos.find((token) => token._id === crypto._id)
          );
        }),
      ];
      this.#details.setCryptos(cryptos);
  ...
  async walletConnect() {
    if (!this.#walletConnect) {
      this.#walletConnect = import("./WalletConnect.js").then(
        ({ WalletConnect }) => {
  ...
  unknownError() {
    if (this.isOnion && navigator.onLine)
      return i18n.global.t("Error! Please ensure that your Tor VPN is active.");
