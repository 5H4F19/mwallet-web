...
export default {
  ...
  methods: {
    async setup() {
      const result = await this.$account.biometry.enable(
        this.storage.pin,
        this.storage.seed
      );
      if (!result) return;
...
<template>
  <AuthStepLayout :title="title" @back="skip">
    <div class="&__icon-wrapper">
      <component :is="icon" class="&__icon" />
    </div>
    <div class="&__text">
      {{ text }}
    </div>
    <CsButtonGroup>
      <CsButton type="primary" @click="setup">
        {{ buttonLabel }}
      </CsButton>
      <CsButton type="primary-link" @click="skip">
        {{ $t("Skip") }}
      </CsButton>
    </CsButtonGroup>
  </AuthStepLayout>
