...
export default {
  ...
  watch: {
    passphrase(passphrase, passphraseOld) {
      ...
      const passphraseDelta = Math.abs(
        passphrase.length - passphraseOld.length
      );
  ...
  methods: {
    async confirm() {
      ...
      try {
        ...
        if (this.$account.isCreated) {
          ...
          if (
            hex.encode(detailsKey) !==
            hex.encode(this.$account.clientStorage.getDetailsKey())
          ) {
            throw new Error();
          }
        }
    ...
    acceptSuggestion(suggestion) {
      this.error = undefined;
      this.passphrase = [...this.words.slice(0, -1), suggestion + " "].join(
        " "
      );
...
<template>
  <div class="&__container">
    <CsFormTextarea
      ...
      :error="error"
    />
    <div v-if="suggestions.length" class="&__suggestions">
      <CsButton
  ...
  <CsButtonGroup class="&__buttons">
    <CsButton type="primary" :isLoading="isLoading" @click="confirm">
      {{ $t("Confirm") }}
    </CsButton>
    <CsButton
      type="primary-link"
      @click="$safeOpen('https://en.wikipedia.org/wiki/Passphrase')"
    >
