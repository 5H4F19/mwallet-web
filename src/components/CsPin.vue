...
export default {
  ...
  data() {
    ...
    return {
      ...
      biometryIcon:
        type === TYPES.FACE_ID ? "FaceIdSolidIcon" : "TouchIdSolidIcon",
    };
  },
  watch: {
    async value(value) {
        ...
        switch (this.mode) {
          ...
          case "deviceSeed": {
            ...
            return await this.onSuccess(
              this.$account.getSeed("device", hex.decode(deviceToken)),
              pin
            );
          }
          case "walletSeed": {
            ...
            if (res.walletToken) {
              return await this.onSuccess(
                this.$account.getSeed("wallet", hex.decode(res.walletToken)),
                pin
              );
            }
            ...
            return await this.onSuccess(
              this.$account.getSeed("wallet", hex.decode(walletToken)),
              pin
            );
          }
        }
...
<template>
  ...
  >
    <span v-if="error && !isLoading" class="&__error">
      {{ error }}
    </span>
    <template v-else>
      <div class="&__dot" :class="{ '&__dot--active': value.length > 0 }" />
      <div class="&__dot" :class="{ '&__dot--active': value.length > 1 }" />
      <div class="&__dot" :class="{ '&__dot--active': value.length > 2 }" />
      <div class="&__dot" :class="{ '&__dot--active': value.length > 3 }" />
    </template>
  </div>
  <div class="&__keyboard">
    ...
    <div class="&__row">
      ...
      <div v-else class="&__key &__key--disabled" />
      <CsButton :key="0" class="&__key" type="secondary" @click="enter(0)">
        0
      </CsButton>
      <CsButton
        v-if="value.length"
      ...
      <CsButton v-else-if="biometryIsEnabled" class="&__key" @click="biometry">
        <component :is="biometryIcon" class="&__biometry" />
      </CsButton>
      <div v-else class="&__key &__key--disabled" />
    </div>
  </div>
