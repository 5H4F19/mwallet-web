  ...
  watch: {
    $cryptos: {
      async handler() {
        ...
        this.portfolioBalanceChangePercent = portfolioBalance
...
<template>
  <div class="&" :class="{ '&--active': active }">
    <div class="&__navbar">
      <CsButton @click="$router.push({ name: 'settings' })">
        <CsAvatar
          class="&__avatar"
          :avatar="$user.avatar"
          own
      ...
      <div
        ...
        :class="{
          '&__portfolio-change--positive':
            !$isHiddenBalance && portfolioBalanceChange > 0,
          '&__portfolio-change--negative':
            !$isHiddenBalance && portfolioBalanceChange < 0,
        }"
      >
    ...
    <div class="&__content">
      <CsCryptoList
        ...
        @select="
          (id) => $router.push({ name: 'crypto', params: { cryptoId: id } })
        "
      />
      <CsButton
        type="primary-link"
...
.#{ $filename } {
  ...
  &__navbar {
    padding: $spacing-3xl max($spacing-xl, env(safe-area-inset-right))
      $spacing-xl max($spacing-xl, env(safe-area-inset-left));
  }
  ...
  &__content {
    ...
    flex: 1 1 100%;
    flex-direction: column;
    padding: $spacing-xl max($spacing-xl, env(safe-area-inset-right))
      $spacing-3xl max($spacing-xl, env(safe-area-inset-left));
    gap: $spacing-2xs;
    @include breakpoint(lg) {
      overflow-y: auto;
    }
